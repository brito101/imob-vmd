                                                ...
                                                <div class="card card-widget widget-user">
                                                    ...
                                                    <div class="widget-user-image">
                                                        @if (!empty($user->url_cover))
                                                            <img class="img-circle elevation-2" src="{{ $user->url_cover }}"
                                                                alt="{{ $user->name }}">
                                                        @else
                                                            <img class="img-circle elevation-2"
                                                                src="{{ url('backend/assets/images/avatar.jpg') }}"
                                                                alt="{{ $user->name }}">
                                                        @endif
                                                    </div>
                                                        ...
                                                        <div class="row">
                                                            <div class="col-sm-12">
                                                                <div class="description-block">
                                                                    <p class="widget-user-username">{{ $user->getRoleNames()->first() }}</p>
                                                                    <p class="widget-user-desc text-sm text-muted">Desde:
                                                                        {{ $user->created_at ? date('d/m/Y', strtotime($user->created_at)) : 'sem data de criação' }}
                                                                    </p>
                                                                    <p class="widget-user-desc text-sm text-muted">Último
                                                                        acesso:
                                                                        {{ $user->last_login_at ? date('d/m/Y H:i:s', strtotime($user->last_login_at)) : 'inexistente' }}
                                                                    </p>
                                                                    <a class="icon-cog btn bg-gradient-lightblue mt-3"
